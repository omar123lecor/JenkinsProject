pipeline {
    ...
    stages {
        ...
        steps {
            script {
                env.DBT_CONTAINER_ID = sh(
                    script: "docker compose run -d dbt",
                    returnStdout: true
                ).trim()
            }

        sh '''
            echo "Copying full project..."
            docker cp dbt/. $DBT_CONTAINER_ID:/usr/app/dbt
            echo "Running dbt run..."
            docker exec $DBT_CONTAINER_ID dbt run
        '''
    }
}
